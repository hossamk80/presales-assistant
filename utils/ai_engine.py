...
#
# `knowledge.style_fingerprint` تقيس شكل عرض سابق، وهذه تحوّل القياس تعليمات.
# الفصل مقصود: القياس لا يعرف النموذج، والصياغة لا تعرف كيف قِيس.
#
# التعليمة تصف **الشكل وحده**. لا تُمرَّر جملة واحدة من العرض السابق ولا اسم
# ولا رقم منه — ما يصل النموذج أرقام مجرّدة عن طول الجملة ونسبة النقاط، وهي
# عاجزة بطبيعتها عن حمل واقعة من منافسة أخرى.

STYLE_MARKER = "أسلوب الكتابة المعتمد لدى الشركة"


def _style_person(share: int) -> str:
    """نسبة صيغة المتكلم الجمع ← وصفها. الحدود ثلاثة لا سلّم متصل."""
    if share >= 60:
        return "بصيغة المتكلم الجمع الصريحة (نحن نلتزم · نقدّم · لدينا)"
    if share >= 25:
        return "بمزيج من صيغة المتكلم الجمع والصياغة الموضوعية"
    return "بصياغة موضوعية غير شخصية (المبني للمجهول والاسمية)"


def _style_density(share: int) -> str:
    if share >= 60:
        return "الغالب نقاط مرقّمة أو منقّطة لا فقرات متصلة"
    if share >= 25:
        return "مزيج متوازن من الفقرات والنقاط"
    return "الغالب فقرات متصلة لا قوائم"


def style_instruction(fingerprint: dict) -> str:
    """
    البصمة مصاغةً تعليمات جاهزة للحقن — أو نصاً فارغاً لبصمة فارغة.

    **الفراغ عند غياب البصمة مقصود**: تعليمة أسلوب مبنية على لا شيء تدفع النموذج
    إلى نبرة مخترعة، وهي أسوأ من تركه على افتراضه المدرَّب.

    والناتج **دالة خالصة من البصمة**: بصمة واحدة تعطي النصّ نفسه حرفاً بحرف في
    كل عرض — وهو شرط قبول 14-5 (نبرة واحدة لا نبرتان متقاربتان).
    """
    if not fingerprint:
        return ""

    lines = [
        f"\n\n--- {STYLE_MARKER} (شكل الكتابة لا مضمونها) ---",
        "التزم بهذه السمات المقيسة من عروض الشركة السابقة:",
        f"- متوسط طول الجملة نحو {fingerprint['sentence_words']} كلمة.",
        f"- الفقرة نحو {fingerprint['paragraph_sentences']} جملة.",
        f"- {_style_density(fingerprint['bullet_share'])}.",
        f"- {_style_person(fingerprint['first_person_share'])}.",
    ]

    levels = fingerprint.get("heading_levels") or []
    if levels:
        depth = "وعنوان فرعي" if len(levels) > 1 else ""
        lines.append(f"- عناوين واضحة لكل جزء{' ' + depth if depth else ''}.")
    if fingerprint.get("uses_tables"):
        lines.append("- استعمل الجداول حيث تُختصر المقارنة أو المواصفات.")

    lines.append(
        "هذه سمات **شكلية**: لا تنقل من العروض السابقة واقعةً ولا رقماً ولا اسم "
        "عميل ولا التزاماً — تلك تخصّ منافسات أخرى."
    )
    return "\n".join(lines)


def has_style_instruction(text: str) -> bool:
...
4. quantity رقم، ومعه **دائماً** quantity_source:
   · "stated" إن وردت الكمية صراحةً في المستندات — انقلها كما هي.
   · "derived" إن لم ترد واشتققتَها حساباً من أرقام أخرى في الكراسة
     (عدد المستخدمين · المساحات · عدد الطوابق · مدة العقد · عدد الفروع).
   واملأ quantity_basis مع derived: **المعادلة ومن أين جاء كل رقم فيها**
   مع مرجع الصفحة أو البند. مثال: «نقطة شبكة لكل موظف · 500 موظف (بند 3-2)».
5. **إن لم تجد في الكراسة ما تشتقّ منه الكمية فضع 1 مع stated ولا تخترع
   أساساً.** رقمٌ واثق بأساسٍ ملفَّق أسوأ من رقمٍ يعرف الفريق أنه مبدئي —
   الأول يُعتمد ويُحاسَب عليه في التنفيذ، والثاني يُراجَع.
   والكميات المشتقّة تُعرَض على مسؤول لاعتمادها ولا تخرج في العرض قبله.
6. specifications و construction_code: انقلهما كما وردا؛ اترك الحقل فارغاً
   إن لم يردا بدل اختراعهما.
7. mandatory_list_flag: ضع true فقط إذا دلّت مستندات المنافسة أو القائمة
   المرجعية المرفقة على أن البند ضمن القائمة الإلزامية للمحتوى المحلي.
   عند الشك ضع false — سيراجعها فريق المشتريات يدوياً.
8. لا تضع أسعاراً لا في الكميات ولا في أساس الاحتساب — التسعير مسؤولية
   الفريق المالي، وهذا مخرَج فني.
9. إن لم يرد جدول كميات صريح، استنتج البنود القابلة للتسعير من نطاق العمل —
   وكمياتها حينئذ derived بأساسها.""",
