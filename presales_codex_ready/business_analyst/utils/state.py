...
# مصدر الكمية — من أين جاء الرقم:
#
#   من الكرّاس  منقول حرفياً، فليس اجتهاداً ولا يحتاج اعتماداً
#   محسوبة     اشتقّها النموذج من أرقام في الكرّاس — **اقتراحٌ حتى يُعتمد**
#   يدوي       كتبها الفريق
#
# التمييز ليس تجميلاً: جدول الكميات يُترجَم إلى مال في المظروف المالي، ورقمٌ
# اشتقّه نموذجٌ من فرضية لا يُقرأ كرقمٍ نقله موظفٌ من صفحةٍ في الكرّاس.
QTY_FROM_TENDER = "من الكرّاس"
QTY_DERIVED = "محسوبة"
QTY_MANUAL = "يدوي"
QTY_SOURCE_OPTIONS = [QTY_FROM_TENDER, QTY_DERIVED, QTY_MANUAL]
...
DEFAULT_BOQ_DF = pd.DataFrame({
    ...
    "الكمية": [1],
    "أساس الاحتساب": [""],
    # الصفّ الفارغ الجاهز للكتابة يدويّ: من يكتب فيه إنسان لا نموذج
    "مصدر الكمية": [QTY_MANUAL],
    "معتمَد": [True],
    ...
    for col in BOQ_COLUMNS:
        if col not in out.columns:
            if col == "الكمية":
                ...
                out[col] = False
            elif col == "مصدر الكمية":
                # جدول محفوظ قبل هذا التوسيع لا يُعرف مصدر أرقامه، ولا يجوز
                # وسمها «محسوبة» فتُحجب عن التصدير فجأةً وقد اعتمدها الفريق.
                out[col] = QTY_FROM_TENDER
            elif col == "معتمَد":
                out[col] = True
            else:
                out[col] = ""
...
# ─── خطّ الأنابيب واستراتيجية العرض (14-8) ────────────────────────────────────
#
# المنافسة كانت إمّا مفتوحة أو مغلقة، بلا موضع بينهما: أين هي من دورة البيع؟
# من يقودها؟ وما احتمال فوزنا بها؟ وأهمّ من الثلاثة: **لماذا نفوز** — الجملة
# التي تُميّزنا، والتي كان كل قسم يُكتب بمعزل عنها فيخرج العرض صحيحاً بلا حجّة.
#
# **وهنا الفصل الذي يحكم هذا البند كلّه**: من الحقول الأربعة **حقل واحد يصل
# النموذج** — «لماذا نفوز». أمّا المرحلة والمالك واحتمال الفوز فبيانات إدارة
# داخلية:
#
#   · **احتمال الفوز رقم لا يُكتب في عرض أبداً.** تسريب «احتمال فوزنا 40٪» إلى
#     نصّ يقرأه المُقيّم كارثة لا تُصلَح، والنموذج لا يُؤتمن على تمييز ما يُذكر
#     ممّا لا يُذكر حين يصله كلاهما في سياق واحد. المنع بنيوي: لا يصل أصلاً.
#   · **المالك اسم موظف** — بيانات شخص، ولا حاجة للنموذج بها لكتابة قسم.
#   · **المرحلة** إدارة داخلية لا تخصّ الجهة.
#
# واحتمال الفوز هنا **تقدير بشري لا قياس**: نسبة الفوز في 14-7 محسوبة من نتائج
# مسجّلة، وهذه رأي صاحب المنافسة قبل حسمها. لا يغذّي أحدهما الآخر، وإلا صارت
# لوحة القياس تعرض ظنّاً بلباس رقم.

PIPELINE_STAGES = (
    "", "رصد", "دراسة الجدوى", "قرار الخوض", "الإعداد", "المراجعة",
    "قُدِّم", "التفاوض", "حُسم",
)

# الحقول التي **لا تصل النموذج أبداً** — يحرسها اختبار.
INTERNAL_PIPELINE_FIELDS = ("pipeline_stage", "pipeline_owner", "win_probability")


def strategy_block() -> str:
    """
    «لماذا نفوز» كمقطع سياق يُحقن مع كل قسم — أو فارغ إن لم تُكتب.

    **حقل واحد من أربعة**: المرحلة والمالك واحتمال الفوز لا تُذكر هنا ولا تصل
    النموذج بأي مسار. انظر تعليق القسم أعلاه.

    والصياغة تطلب **أثراً لا ترديداً**: نسخ جملة الاستراتيجية حرفياً في كل قسم
    يجعل العرض يكرّر شعاراً، والمطلوب أن ينعكس المعنى في اختيار ما يُبرَز.
    """
    why = str(st.session_state.get("why_we_win", "") or "").strip()
    if not why:
        return ""
    return (
        "\n\n--- استراتيجية العرض: لماذا نفوز بهذه المنافسة ---\n"
        f"{why}\n"
        "اجعل هذه الحجّة تظهر في اختيار ما تُبرزه وما تُفصّله في هذا القسم، "
        "ولا تنسخها جملةً مكرَّرة في كل قسم — الأثر مطلوب لا الترديد. وما لا "
        "يسنده دليل في السياق لا يُدّعى ولو خدم الحجّة."
    )
...
STATE_SCHEMA = {
    ...
    "project_sector": "",
    # خطّ الأنابيب واستراتيجية العرض (14-8). «لماذا نفوز» وحدها تصل النموذج،
    # والثلاثة الأخرى إدارة داخلية — انظر `strategy_block`.
    "pipeline_stage": "",
    "pipeline_owner": "",
    "win_probability": 0,
    "why_we_win": "",
