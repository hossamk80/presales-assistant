...
def open_project(project_id: int):
    ...
    if record is None:
        st.error("❌ لم يُعثر على المنافسة.")
        return
...
def render():
    st.markdown("### 📁 المنافسات المحفوظة")
    st.caption(
        "كل منافسة تُحفظ على القرص محلياً مع تحليلاتها وأقسامها وجداولها. "
        f"ملف قاعدة البيانات: `{db.DB_PATH}`"
    )
    ...
    if pid is not None:
        ...
        with c1:
            st.success(f"📂 المنافسة المفتوحة: **{current_project_name()}**")
        with c2:
            if st.button("💾 حفظ الآن", width="stretch"):
                save_current(show_toast=True)
                st.rerun()
        with c3:
            if st.button("📕 إغلاق", width="stretch"):
                close_project()
                st.rerun()
    else:
        st.info("لا توجد منافسة مفتوحة. أنشئ واحدة جديدة أو افتح محفوظة أدناه.")

    if st.session_state.get("_autosave_error"):
        st.warning(f"⚠️ تعذّر الحفظ التلقائي: {st.session_state.pop('_autosave_error')}")

    st.divider()

    # ── إنشاء منافسة ──────────────────────────────────────────────────────────
    with st.expander("➕ منافسة جديدة", expanded=pid is None):
        with st.form("new_project", clear_on_submit=True):
            c1, c2 = st.columns(2)
            with c1:
                name = st.text_input("اسم المنافسة *", placeholder="تطوير بوابة إلكترونية")
                entity = st.text_input("الجهة", placeholder="وزارة ...")
            with c2:
                reference = st.text_input("رقم المنافسة", placeholder="2026-...")
                carry = st.checkbox(
                    "انقل بيانات الجلسة الحالية إلى المنافسة الجديدة", value=False,
                    help="مفيد إذا بدأت العمل قبل إنشاء المنافسة.",
                )
            if st.form_submit_button("➕ أنشئ المنافسة", type="primary"):
                if not name.strip():
                    st.error("أدخل اسم المنافسة.")
                else:
                    save_current()
                    if not carry:
                        _clear_project_state()
                    ...
                    st.session_state["_saved_fingerprint"] = _fingerprint(get_project_snapshot())
                    st.success(f"✅ أُنشئت المنافسة «{name.strip()}».")
                    st.rerun()
    ...
    st.markdown(f"**{len(projects)} منافسة محفوظة**")
        ...
        with st.container(border=True):
            ...
            with c_info:
                meta = " · ".join(filter(None, [
                    proj["entity"], proj["reference"], f"آخر تحديث: {proj['updated_at']}"
                ]))
                st.markdown(
            ...
            with c_open:
                if st.button("فتح", key=f"open_{proj['id']}", width="stretch", disabled=is_open):
                    open_project(proj["id"])
                    st.rerun()
            with c_dup:
                if st.button("نسخ", key=f"dup_{proj['id']}", width="stretch"):
                    db.duplicate_project(proj["id"], f"{proj['name']} (نسخة)")
                    st.rerun()
            with c_del:
                confirm_key = f"confirm_del_{proj['id']}"
                if st.session_state.get(confirm_key):
                    if st.button("تأكيد؟", key=f"del2_{proj['id']}", width="stretch", type="primary"):
                        if is_open:
                            close_project()
                    ...
                    if st.button("حذف", key=f"del_{proj['id']}", width="stretch"):
